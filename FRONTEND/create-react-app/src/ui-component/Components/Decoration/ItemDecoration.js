...
function UIColor(props) {
  ...
  useEffect(() => {
    const apiProdUrl = `http://127.0.0.1:8000/store/Location/Decoration/${props.id}`;
  ...
  return (
    <Grid data-aos="fade-left" container spacing={gridSpacing} style={{ margin: '5px' }}>
      <Grid
        style={{ backgroundImage: `url(${produit.image})` }}
        item
        xs={12}
        sm={6}
        md={4}
        lg={2}
        key={produit.id}
        className={`conteneurProduit rounded-1 ${classedPerImage(produit.image)}`}
        onClick={() => handleFormPanier(produit)}
      >
        <ColorBox bgcolor="grey.50" data={{ label: produit.nom, prix: produit.prix_unitaire + " Ar" }} title="" dark />
      </Grid>
    </Grid>
  );
}
