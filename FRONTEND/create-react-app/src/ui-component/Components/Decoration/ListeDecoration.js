...
import FormProduit from 'ui-component/Components/Decoration/Forms/FormProduit';
import FormCategorie from 'ui-component/Components/Decoration/Forms/FormCategorie';
...
function Produit() {
  ...
  const produitsParCategorie = donneesProduit.reduce((acc, produit) => {
    const typeProduitId = produit.categorie;
  ...
  const handleDeleteCategorie = (categoryId) => {
    const apiUrl = `http://127.0.0.1:8000/store/Location/TypeDeco/${categoryId}`;
        ...
        <Grid item xs={12}>
          {donneesType.map((type) => (
           <div key={type.id}>
              ...
              <FormProduit id_cat={categorie.id} submitted={submittedProduit} setSubmitted={setSubmittedProduit} categorie={categorie.titre} />
            </FormModal>
              <SubCard  secondary={<Button style={{color:"red", fontSize:"15px"}} onClick={() => handleDeleteCategorie(type.id)}>x</Button>} className="mb-4-none" data-aos="zoom-in" title={type.titre}>
                  ...
                  {produitsParCategorie[type.id] &&
                    produitsParCategorie[type.id].map((produit) => (
                      <Grid onClick={()=>ShowProfile(produit.id)} style={{backgroundImage:`url(${produit.image})`}} item xs={12} sm={6} md={4} lg={2} key={produit.id} className={`conteneurProduit rounded-1 ${classedPerImage(produit.image)}`}>
                        <ColorBox data={{ label: produit.nom, prix: produit.prix_unitaire.toLocaleString()+" Ar"}} title="" dark />
                      </Grid>
                    ))}
