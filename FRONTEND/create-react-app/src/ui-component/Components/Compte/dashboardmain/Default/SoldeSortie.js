...
const TotalOrderLineChartCard = ({ isLoading, data, flux, setShowSortie }) => {
  const theme = useTheme();
  const [montant, setMontant]=useState(0);
  const sorties=flux.filter(donnees=>donnees.debiteur===data.id)

  const [anchorEl, setAnchorEl] = useState(null);

  useEffect(() => {
    setMontant(sorties.reduce((total, donnees)=>total+donnees.montant,0));
  }, [sorties]);

  const formatSolde = (dt) => {
    return dt.toLocaleString();  // Utilisez la fonction toLocaleString() pour le formatage
  };
              ...
              <Grid item>
                <Grid container justifyContent="space-between">
                  <Grid item>
                    ...
                    >
                      <EarningIcon onClick={()=>setShowSortie(data.id)} style={{color:"white"}} fontSize="inherit" />
                    </Avatar>
                  </Grid>
                  <Grid item>
                    <Avatar
