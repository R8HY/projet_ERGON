...
function App({packParam, setPackage}) {
  ...
  const [showAllCheckBox, setShowAllCheckBox] = useState(packParam===false);
  const [selectedPackage, setSelectedPackage] = useState(packParam===false?{}:packParam); 
    ...
    const selectPackage = (cl) => {
        setSelectedPackage(cl);
        setShowAllCheckBox(!showAllCheckBox);
        if(showAllCheckBox===true)setPackage(cl);
        else setPackage(false);
    }
                  ...
                  <MDBRow>
                    <div className="text-center mt-1">
                      ...
                      <h6 className="text-primary mb-3 pb-4">{salles
                        .filter((prod) => prod.id == item.salle)
                        .map((salle, index) => (<h4 key={index} style={{color:"#666"}}>Salle : {salle.nom}</h4>))}<br/>{item.description}</h6>
                    </div>
                  </MDBRow>
                  <MDBRow>
                ...
                <div>
                  ...
                  <div className="d-flex flex-column">
                    <Button variant="primary" size="sm">
                      ...
                      <div style={{display:"flex-end", marginTop:"100%"}}>
                        {
                        (!showAllCheckBox)?
                        (
                            selectedPackage.id===item.id?
                            (
                                <input className="form-check-input" type="checkbox" value="" checked={!(packParam===false)} onChange={()=>{selectPackage(item)}} style={{float:"right", width:"25px", height:"25px"}}/>
                            ):(<></>)
                        )  
                        :
                        (<input className="form-check-input" type="checkbox" value="" onChange={()=>{selectPackage(item)}} style={{float:"right", width:"25px", height:"25px"}}/>)
                        }
                    </div>
                  </div>
                </div>
